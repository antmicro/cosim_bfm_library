...
{
    cc_printf(1,"trying to init barrier: channel %d for host.\n", chan_id);
    if (chan_id>=MAX_NUM_CHAN) return -1;
    if (chan_handle[chan_id]==(void*)-1L) return -1;
    char bmsgTx[64];
    char bmsgRx[64];
    ...
    cc_printf(1,"trying to init barrier: channel %d for host. (2)\n", chan_id);
    if (side_ht[chan_id]==CHAN_HOST) {
        ...
        cc_printf(1,"chn_send: channel %d for host.\n", chan_id);
        //--------------------------------------------------------------------
        ...
        cc_printf(1,"chn_recv: channel %d for host.\n", chan_id);
